...
def generar_pdf():
    ...
    sMetricN = S('mn',     fontName='Helvetica-Bold', fontSize=20, textColor=RED,   alignment=TA_CENTER, leading=24)
    ...
    ht = Table([[
        ...
        Paragraph(f'ICBC Mall<br/><font size="9">{fecha}</font>',
    ...
    story.append(Spacer(1,4))
    story.append(Paragraph(f'Reporte generado el {fecha} — ICBC Mall · Datos internos', sFooter))

    doc.build(story)
    buffer.seek(0)
    return send_file(buffer, mimetype='application/pdf',
                     as_attachment=True,
                     download_name=f'reporte_icbc_{fecha}.pdf')
